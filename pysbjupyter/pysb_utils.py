...
def parse_name(spec):
    ...
    for i in range(len(m)):
        ...
        tmp_2 = re.findall(r"['\"](.*?)['\"]",
                           str(m[i]))  # Matches strings between quotes
...
def rate_2_interactions(model, rate):
    ...
    if len(species_idxs) == 1:
        ...
        sp_monomers = {sp: model.species[sp].monomer_patterns for sp in
                       species_idxs}
        sorted_intn = sorted(sp_monomers.items(),
                             key=lambda value: len(value[1]))
        interaction = " ".join(
            parse_name(model.species[mons[0]]) for mons in sorted_intn[:2])
    return interaction
...
def find_nonimportant_nodes(model):
    ...
    non_imp_rcts = set(
        [x for x in range(len(model.species)) if rcts_sp.count(x) < 2])
    non_imp_pdts = set(
        [x for x in range(len(model.species)) if pdts_sp.count(x) < 2])
